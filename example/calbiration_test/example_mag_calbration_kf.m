...
% 修改：转置数据，使每行代表一个样本
dataset.imu.gyr = [T.gyr_x T.gyr_y T.gyr_z];
dataset.imu.mag = [T.mag_x T.mag_y T.mag_z];

N = size(dataset.imu.gyr, 1);

% 不使用全部数据集，只使用一部分
ratio = 0.9;

cal_len = floor(ratio*N);

cal_gyr = dataset.imu.gyr(1:cal_len, :);
cal_mag = dataset.imu.mag(1:cal_len, :);
val_mag = dataset.imu.mag(cal_len+1:end, :);
