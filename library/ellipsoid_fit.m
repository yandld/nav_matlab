function [center, gain, rotM, evecs, v, chi2]=ellipsoid_fit(XYZ,varargin)
    ...
    if f==0, % rotated ellipsoid
     ...
     center=-A(1:3,1:3)\[v(7);v(8);v(9)]; % offset is center of ellipsoid
     Tmtx=eye(4); Tmtx(4,1:3)=center'; AT=Tmtx*A*Tmtx'; % ellipsoid translated to (0,0,0)
     ...
     center=-(v(1:3).\v(7:9))'; % offset is center of ellipsoid
    ...
    d = [ d(:,1) / gain(1), d(:,2) / gain(2), d(:,3) / gain(3) ]; % normalize to the conic radii
