...
opt.P0 = diag([ [2 2 10]*D2R, [1 1 1], [5 5 5],  [50 50 50]* D2R/3600, [1e-3 1e-3 1e-3]*GRAVITY, [45 45 45]*D2R])^2;
% 系统方差:       角度随机游走           速度随机游走                      角速度随机游走        加速度随机游走
opt.Q = diag([(5/60*D2R)*ones(1,3), (4/60)*ones(1,3), 0*ones(1,3), [2.0 2.0 2.0]/3600*D2R, 0*GRAVITY*ones(1,3), 0,0,0])^2;
...
cq = angle2quat(0, 0, 0, 'ZXY');
C = angle2dcm(0,0,0,'ZXY');
nhc_angle = zeros(imu_length,3);
nhc_bit = 0;
...
log.P = zeros(imu_length, N);
log.X = zeros(imu_length, N);
...
%                 GNSS_R = GNSS_R/lambda;
       ...
       %     end
        ...
        if norm(vel_data(i,:))>5
            H = zeros(3,N);
            H(1:3, 16:18) = v3_skew([0 1 0]);

            vn = vel_data(i,:)'/norm(vel_data(i,:));
            vb = nCb*vn;
            Z = C*vb - [0;1;0];
...
%% 姿态与航向估计曲线
gv_speed = zeros(imu_length,1);
gv_yaw = zeros(imu_length,1);
for i=1:imu_length
    gv_yaw(i) = atan2d(vel_data(i,1),vel_data(i,2));
    gv_yaw(i) = gv_yaw(i) + (gv_yaw(i)<0)*360;

    gv_speed(i) = norm(vel_data(i,:));
end
figure; subplot(2,1,1); plot(gv_speed); subplot(2,1,2); plot(gv_yaw);

mcu.att(:,3) = -mcu.att(:,3);
mcu.att(:,3) = mcu.att(:,3) + (mcu.att(:,3)<0)*360;
plot_att(imu_time,log.att, mcu_time,mcu.att, imu_time,log.sins_att, imu_time,[bl_pitch gv_yaw]);

%%
figure;
for i=1:3
    subplot(3,1,i);
    plot(imu_time, nhc_angle(:,i), 'LineWidth', 1.5); grid on;
