...
opt.P0 = diag([ [2 2 10]*D2R, [1 1 1], [5 5 5],  [50 50 50]* D2R/3600, [1e-3 1e-3 1e-3]*GRAVITY, 90])^2;
% 系统方差:       角度随机游走           速度随机游走                      角速度随机游走        加速度随机游走
opt.Q = diag([(5/60*D2R)*ones(1,3), (4/60)*ones(1,3), 0*ones(1,3), [2.0 2.0 2.0]/3600*D2R, 0*GRAVITY*ones(1,3), 0])^2;
...
%                 GNSS_R = GNSS_R/lambda;
       ...
       %     end
        ...
        if norm(vel_data(i,:))>5
            ...
            speed_yaw = atan2d(vel_data(i,1),vel_data(i,2));
            speed_yaw = speed_yaw + (speed_yaw<0)*360;

            Z = [atand(tand(yaw - speed_yaw))];
