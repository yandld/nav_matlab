...
%P阵收敛曲线
    ...
    xlabel('时间(分钟)'); ylabel('平台失准角(°)'); legend('Pitch', 'Roll', 'Yaw');
    subplot(3,2,2);
    semilogy(time, P(:, 4:6), 'linewidth', 1.5); grid on;
    xlim([time(1) time(end)]);
    xlabel('时间(分钟)'); ylabel('速度误差(m/s)'); legend('Ve', 'Vn', 'Vu');
    subplot(3,2,4);
    semilogy(time, P(:, 7:9), 'linewidth', 1.5); grid on;
    xlim([time(1) time(end)]);
    xlabel('时间(分钟)'); ylabel('位置误差(m)'); legend('Lat', 'Lon', 'Alt');
    subplot(3,2,3);
    semilogy(time, P(:, 10:12) * 3600 * 180/pi, 'linewidth', 1.5); grid on;
    xlim([time(1) time(end)]);
    xlabel('时间(分钟)'); ylabel('陀螺零偏(°/h)'); legend('X', 'Y', 'Z');
    subplot(3,2,5);
    semilogy(time, P(:, 13:15) / 9.8 * 1000, 'linewidth', 1.5); grid on;
    xlim([time(1) time(end)]);
    xlabel('时间(分钟)'); ylabel('加速度计零偏(mg)'); legend('X', 'Y', 'Z');
